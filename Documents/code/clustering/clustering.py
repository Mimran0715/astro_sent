...
from text_process_c import get_data
...
def get_lda(df):
    ...
    #df = df_2
    #getting data
    train, test, count_vect, X_tr_count, count_terms, X_te_count = vect_data(df, 1)

    texts = list(train['abs_text'])
    tagged = []
    for text in texts:
...
def kmeans_clustering(df, results_dir, kmeans_dir, c):    
    df = get_sent(df)
    ...
    # X_te_tfidf = vect.transform(test['abstract'])
    # X_te_count = count_vect.transform(test['abstract'])
    train, test, vect, X_tr_tfidf, tfidf_terms, X_te_tfidf = vect_data(df, 0)
    ...
    #wordcloud_pdf = PdfPages(results_dir + "wordcloud.pdf")
    #df = temp
...
def main():
    ...
    df = get_data(db_path, tbl_name, size)
    kmeans_clustering(df, results_dir, kmeans_dir)
    get_lda(df)
