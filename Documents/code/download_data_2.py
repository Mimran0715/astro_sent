...
def obtain_ads_data(start: int, params: dict, db: int, f_path:str, db_path:str, table_name:str, fields:list, rows:int) -> None:
    ...
    #while ads_response['response']['numFound'] != 0:
        ...
        for paper in papers:
            ...
            #values = list(val_dict.values()) + ["", 0, 0, 0.0, "", "", ""]
            ...
            try:
                if db == 0:
                    db_command(db_path, cmd_str, values)
                    #db_command(db_path, cmd_str, (paper['bibcode'], paper['title'][0], int(paper['year']), \
                    #        ' '.join(paper['author']), paper['abstract'], int(paper['citation_count']), file_name, 1, 0, 0.0, text, None))
            except:
